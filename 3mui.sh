...
# 检查端口占用情况
check_port_conflict() {
    echo -e "${YELLOW}>>> 检查 ${DEFAULT_PORT} 端口是否被占用...${PLAIN}"
    if ss -tulpn | grep -q ":${DEFAULT_PORT} "; then
        echo -e "${RED}❌ 严重错误：发现 ${DEFAULT_PORT} 端口已被占用！${PLAIN}"
        echo -e "${YELLOW}占用该端口的进程信息如下：${PLAIN}"
        ss -tulpn | grep ":${DEFAULT_PORT} "
        echo -e ""
        echo -e "${RED}由于 3m-ui 首次启动强制要求绑定 ${DEFAULT_PORT} 端口（且使用了 host 网络），${PLAIN}"
        echo -e "${RED}请先停止上述冲突的进程，释放端口后再来执行安装！${PLAIN}"
        return 1
    fi
    echo -e "${GREEN}✅ ${DEFAULT_PORT} 端口空闲，环境正常。${PLAIN}"
    return 0
}
...
show_info() {
    ...
    # 自动精准提取初始密码
    INIT_PWD=$(docker logs 3m-ui 2>&1 | grep "Initial password:" | tail -n 1 | awk '{print $3}')
    if [[ -z "${INIT_PWD}" ]]; then
        PWD_DISPLAY="${RED}未抓取到或你已修改密码 (若遗忘可按 7 重置)${PLAIN}"
    else
        PWD_DISPLAY="${GREEN}${INIT_PWD}${PLAIN}"
    fi
    ...
    echo -e " 🔑 ${YELLOW}初始随机密码 :${PLAIN} ${PWD_DISPLAY}"
    echo -e "${GREEN}==================================================================${PLAIN}"
    echo -e "${RED}注意: 首次登录后，请务必在面板【设置】中修改密码，并可顺便将端口改为 12053！${PLAIN}"
}
...
install_panel() {
    ...
    # 前置端口冲突拦截
    check_port_conflict || return
    
    echo -e "${GREEN}>>> 正在创建 Docker 命名卷...${PLAIN}"
    docker volume create ${VOL_CONFIG} >/dev/null 2>&1
    docker volume create ${VOL_DATA} >/dev/null 2>&1
    
    echo -e "${YELLOW}>>> 正在拉取官方镜像...${PLAIN}"
    ...
    echo -e "${YELLOW}>>> 正在等待面板初始化并生成密码 (约 5 秒)...${PLAIN}"
    sleep 5
...
update_panel() {
    ...
    if [ $? -eq 0 ]; then
        echo -e "${GREEN}🎉 更新完成！底层数据已通过卷安全保留。${PLAIN}"
    else
        echo -e "${RED}启动容器失败！${PLAIN}"
    fi
}
...
uninstall_panel() {
    ...
    read -p "是否要【彻底删除】数据持久化卷（包含节点配置）？ [y/N]: " delete_data
...
control_panel() {
    local action=$1
    if ! docker ps -a --format '{{.Names}}' | grep -Eq "^3m-ui$"; then
        echo -e "${RED}未检测到 3m-ui 容器！${PLAIN}"
        return
    fi
    
    if [ "$action" == "stop" ]; then
        docker stop 3m-ui >/dev/null 2>&1
        echo -e "${GREEN}✅ 面板已停止运行。${PLAIN}"
...
show_menu() {
    while true; do
        clear
        echo -e "${GREEN}================================================${PLAIN}"
        echo -e "${GREEN}     3m-ui 官方规范一键部署脚本 (智能防冲突版)${PLAIN}"
        echo -e "${GREEN}================================================${PLAIN}"
        echo -e "${YELLOW}1.${PLAIN} 安装 3m-ui 容器"
        echo -e "${YELLOW}2.${PLAIN} 更新 3m-ui 容器 (保留数据)"
        echo -e "${YELLOW}3.${PLAIN} 查看 3m-ui 面板信息与当前密码"
        echo -e "${YELLOW}4.${PLAIN} 卸载 3m-ui 容器"
        echo -e "${GREEN}------------------------------------------------${PLAIN}"
        echo -e "${YELLOW}5.${PLAIN} 停止 3m-ui 容器"
        echo -e "${YELLOW}6.${PLAIN} 重启 3m-ui 容器"
        echo -e "${YELLOW}7.${PLAIN} 忘记密码？一键重置管理员账号"
        ...
        case "${choice}" in
            ...
            5) control_panel "stop"; read -p "按回车键返回主菜单..." ;;
            6) control_panel "restart"; read -p "按回车键返回主菜单..." ;;
            7) docker exec 3m-ui /usr/local/bin/3m-ui reset-admin; read -p "按回车键返回主菜单..." ;;
            0) exit 0 ;;
            *) sleep 1 ;;
        esac
    done
